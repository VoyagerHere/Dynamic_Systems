...
for i = 1:length(Y(:,1))
if(i+1 < length(Y(:,1)))
if(abs(Y(i+1,1) - Y(i,1)) > (2*pi - 0.5))
cutEnd1 = i;
yFinal1 = Y(cutStart1:cutEnd1,1);
for j = cutStart1:cutEnd1
...
if(cutPoint ~= -1)
plot(yFinal1(1 : cutPoint - cutStart1) , Y(cutStart1:cutPoint-1,2),'-r');
plot(yFinal1(cutPoint - cutStart1 + 2 : end), Y(cutPoint + 1:cutEnd1,2),'-r');
else
plot(yFinal1, Y(cutStart1:cutEnd1,2),'-r');
end
cutPoint = -1;
cutStart1 = cutEnd1 + 1;
end
...
plot(Y(cutPoint + 1:cutEnd2,1),yFinal2(cutPoint - cutStart2 + 2 : end),'-r');
else

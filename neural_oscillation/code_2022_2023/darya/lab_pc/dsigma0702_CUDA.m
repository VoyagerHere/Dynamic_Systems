...
% Table with gamma
sigma_list = gpuArray(linspace(0, pi, 20));
% sigma_list = linspace(0, 3, 1);
SIGMA_TABLE = [];
SIGMA_TABLE = sigma_iterate(g1, g2, n, k, sigma_list, SIGMA_TABLE);
close all;
...
function SIGMA_TABLE = sigma_iterate(g1, g2, n, k, sigma_list, SIGMA_TABLE)
    delta_phi_error = 0.05; % phase sync tolerance
    d_list = 0:0.0005:0.5; 
    num_of_iterations = length(sigma_list); 
    close all;
    for z = 1:length(sigma_list)
        sigma = sigma_list(z);
        D_TABLE = d_iterate([g1; g2], d_list, n, sigma, k, delta_phi_error);
...
function DELTA_TABLE = d_iterate(g, d_list, n, sigma, k, delta_phi_error)
   ...
   parfor m = 1:length(d_list)
        ...
        F2_0 = 0;
        F = [F1_0; F2_0];
        [T, Y] = odeK4(@eqn ,[a,b-step], y0, step,[42, 1] ,{d, n, sigma, k}, 0, 0);
        T = transpose(T);
        A1 = Y(:,1); % Phi_1 values
        A2 = Y(:,2); % Phi_2 values
...
function [dy_dt, F] = eqn(~, y, d, no, sigma, k)
  yy = mod(y, 2*pi);

  I = 1./(1 + [exp(k*(cos(sigma) - sin(y(1)))) exp(k*(cos(sigma) - sin(y(2))))]);

...
function SIGMA_TABLE = sigma_iterate(g1, g2, n, sigma_list, SIGMA_TABLE)
    ...
    close all;
    fig = uifigure;
    zs = uiprogressdlg(fig,'Title','Please Wait',...
        'Message','Solving the equation'); % Set progress bar
    drawnow;
    for k = 1:length(sigma_list)
        ...
        zs.Value = k/num_of_iterations; % Update progress bar
        zs.Message = strcat('Iteration: ',num2str(k),' of  ',num2str(num_of_iterations));
    end
end

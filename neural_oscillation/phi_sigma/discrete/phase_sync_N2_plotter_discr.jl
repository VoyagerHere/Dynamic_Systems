...
@load "ph_discr_sin110.005__20240603_2247.jld2" DATA SYNC
...
for ratio in unique_ratios
  ...
  if (ratio == 0)
    if (length(ratio_ind) > accuracy)
      scatter(SIGMA_VEC[ratio_ind], D_VEC[ratio_ind], s=size_sc, color="lavender", label=L"$S$%$counter_field: Q-P")
  ...
  elseif (ratio > 0)
    ...
    if (length(Spike_p) > accuracy) || (length(Burst_p) > accuracy)
      if (Spike_p == Burst_p) 
        ...
        scatter(SIGMA_VEC[Spike_p], D_VEC[Spike_p],  s=size_sc, color=spike_color,label=L"$S$%$counter_field: SS 1:%$ratio")
        global counter_field+=1
      else
        if ((length(Burst_p) > accuracy))
          burst_color = cmap_brst(0.2+(0.5/ratio)) # Normalize the length to a value between 0 and 1

          scatter(SIGMA_VEC[Burst_p], D_VEC[Burst_p],  s=size_sc, color=burst_color, label=L"$S$%$counter_field: BS 1:%$ratio") 
          global counter_field+=1
        end
    ...
    if (length(Spike_p) > accuracy) || (length(Burst_p) > accuracy)
      if (Spike_p == Burst_p) 
        ...
        scatter(SIGMA_VEC[Spike_p], D_VEC[Spike_p],  s=size_sc, color=spike_color,label=L"$S$%$counter_field: SS S-P 1:%$ratio")
        global counter_field+=1
      else
        if ((length(Burst_p) > accuracy))
          burst_color = cmap_brst(0.2+(0.5/ratio)) # Normalize the length to a value between 0 and 1

          scatter(SIGMA_VEC[Burst_p], D_VEC[Burst_p],  s=size_sc, color=burst_color, label=L"$S$%$counter_field: BS S-P 1:%$ratio") 
          global counter_field+=1
        end
        if ((length(Spike_p) > accuracy))
          spike_color = cmap_sp(1.0-(1.0/ratio)) # Normalize the length to a value between 0 and 1
  ...
  elseif (ratio == -1)
    if (length(ratio_ind) > accuracy)
      scatter(SIGMA_VEC[ratio_ind], D_VEC[ratio_ind],  s=size_sc, color=cmap_dth(0.4), label=L"$S$%$counter_field: D $\varphi_1$")
      global counter_field += 1;
    end
  elseif (ratio == -2)
    if (length(ratio_ind) > accuracy)
      scatter(SIGMA_VEC[ratio_ind], D_VEC[ratio_ind],  s=size_sc, color=cmap_dth(0.6), label=L"$S$%$counter_field: D $\varphi_2$")
      global counter_field +=1;
    end
  elseif (ratio == -3)
    if (length(ratio_ind) > accuracy)
      scatter(SIGMA_VEC[ratio_ind], D_VEC[ratio_ind],  s=size_sc, color=cmap_dth(0.8), label=L"$S$%$counter_field: D $\varphi_1, \varphi_2$")
      global counter_field +=1;
    end
  end
end
...
# xlim(0,  0.4)
title(L"$n_1$ = %$N1, $n_2$ = %$N2", fontsize=20)
xlabel(L"\sigma", fontsize=20)
ylabel(L"d", fontsize=20)